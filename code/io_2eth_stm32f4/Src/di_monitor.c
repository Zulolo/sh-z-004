...
//static DI_ConfTypeDef tDI_Conf[SH_Z_004_DI_NUM];
static uint32_t unDI_CNT_FreqValue[SH_Z_004_DI_NUM];
// limitation can only use pins in same port, e.g. GPIOD 
// Also the pins should be continuously
static GPIO_TypeDef* DI_Port = GPIOD;
static const uint16_t DI_Pins[SH_Z_004_DI_NUM] = {DI_0_Pin, DI_1_Pin, DI_2_Pin, DI_3_Pin};
...
static uint32_t get_DI_values(void) {
	...
	assert_param(IS_DI_PIN_NUM(SH_Z_004_DI_NUM));
	
	for (unIndex = 0; unIndex < SH_Z_004_DI_NUM; unIndex++) {
...
uint32_t DI_get_DI_values(void) {
	...
	unTemp = unDI_Value >> SH_Z_004_DI_PIN_OFFSET;
...
uint32_t DI_get_DI_enable_CNT(void) {
	uint32_t unTemp;
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	unTemp = DI_EnableCNT >> SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	return unTemp;
}
...
uint32_t DI_get_DI_CNT_overflow(void) {
	uint32_t unTemp;
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	unTemp = DI_CNT_Overflow >> SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	return unTemp;
}
...
uint32_t DI_get_DI_latch_set(void) {
	uint32_t unTemp;
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	unTemp = DI_LatchSet >> SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	return unTemp;
}
...
uint32_t DI_get_DI_latch_status(void) {
	uint32_t unTemp;
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	unTemp = DI_LatchStatus >> SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	return unTemp;
}
...
void DI_set_DI_latch_status(uint32_t unValue) {
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	DI_LatchStatus = unValue << SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);	
}

void DI_set_DI_enable_CNT(uint32_t unValue) {
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	DI_EnableCNT = unValue << SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	di_save_conf();
}
...
void DI_set_DI_latch_set(uint32_t unValue) {
	osMutexWait(DI_DataAccessHandle, osWaitForever);
	DI_LatchSet = unValue << SH_Z_004_DI_PIN_OFFSET;
	osMutexRelease(DI_DataAccessHandle);
	di_save_conf();
}
...
//	uint8_t* pTestBuf;
	...
	static uint16_t unDI_FilterTimer[SH_Z_004_DI_NUM];
